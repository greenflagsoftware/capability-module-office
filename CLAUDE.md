...
1. Add the capability as a CLI command under `src/AgentDock.Office.Cli/Commands/` (see
   `DocxCommand.cs` for the pattern), exercised standalone before any MCP involvement.
2. Add a class under `src/AgentDock.Office/Tools/` (one file per tool or tool group),
...
3. Update `tools` in [module.manifest.json](src/AgentDock.Office/module.manifest.json) to match.
4. Add CLI-layer tests under `tests/AgentDock.Office.Cli.Tests/` (unit tests against the CLI's
   internals — see `PathSecurityTests.cs`/`DocxEngineTests.cs`) and MCP-adapter tests under
   `tests/AgentDock.Office.Tests/` (see `CliRunnerTests.cs`).
...
src/AgentDock.Office.Cli/        Standalone CLI — every capability's actual implementation
...
tests/AgentDock.Office.Tests/     Tests for the MCP adapter layer (CliRunner, DocxTools)
